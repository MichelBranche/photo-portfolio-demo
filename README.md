...
1. **Hosting statico** (Vercel, Netlify, Cloudflare Pages, ecc.): build command `npm run build`, output directory `dist/`.
...
- **`FashionGallery`** — cuore logico dell'app (griglia, zoom, viste progetto).
- Sistema multi-progetto (14 serie fotografiche).
- Gestione dinamica immagini (local + Google Drive opzionale).
- Layout differenziati per ogni progetto.

## UX / design

- Griglia dinamica editoriale con drag.
- Zoom immagini (GSAP) con sfondo sfocato.
- Slideshow per cella.
- Layout dedicati per serie (editoriale, moda, eventi, ecc.).
- Estetica brutalist con texture e layering visivo.
...
**Limiti:** Drive non è una CDN: con molte immagini puoi avere **più latenza** o limiti rispetto ai file in `public/media/`. Se un'immagine non si vede, verifica condivisione e accesso in lettura.

### Manifest Drive (consigliato)
...
**Michel Branche** — Frontend Developer & Creative
...
Vedi `LICENSE.txt`. Uso non autorizzato, riproduzione o distribuzione vietati salvo accordo.
